...
def draw_plot():
    ...
    sr1 = pd.Series([i for i in range(1880, 2051)])
    ...
    slope, intercept, r, p, std_err = linregress(df['Year'][df['Year'].between(2000, 2013)],
                                                 df['CSIRO Adjusted Sea Level'][df['Year'].between(2000, 2013)])
    plt.plot(df['Year'][df['Year'] >= 2000], intercept + slope * df['Year'][df['Year'] >= 2000], 'r')
